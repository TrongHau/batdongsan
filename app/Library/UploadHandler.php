...
class UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->response = array();
        $this->options = array(
            'script_url' => $this->get_full_url().'/upload/image',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/storage/temp/',
...
//        $dest_y = $watermark[1] - $img->height() - 15;
        ...
        if($ranW >= $img->width() - $watermark->width())
            $ranW = $ranW - 509;
        if($ranH >= $img->height() - $watermark->height())
            $ranH = $ranH - 300;
